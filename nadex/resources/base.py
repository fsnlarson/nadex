    ...
    def _create_object(cls, response, connection=None):
        if isinstance(response, list):
            ...
            if 'hierarchy' in response:
                return cls._create_object(response['hierarchy'], connection)
            elif 'markets' in response:
                return cls._create_object(response['markets'], connection)
            else:
...
class CreatableApiSubResource(ApiSubResource):
    @classmethod
    def _create_path(cls, parentid, id=None):
        if id:
            return "%s/%s/%s/%s" % (cls.parent_resource, parentid, cls.resource_name, id)
        return "%s/%s/%s" % (cls.parent_resource, parentid, cls.resource_name)

    @classmethod
    def create(cls, parentid, id=None, connection=None, **params):
        response = cls._make_request('POST', cls._create_path(parentid, id), connection, data=params)
        return cls._create_object(response, connection=connection)
